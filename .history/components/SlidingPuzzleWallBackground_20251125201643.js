...
const SlidingPuzzleWallBackground = () => {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.FogExp2("#050507", 0.035);

    // Camera - orthographic-ish perspective for wall effect
    ...
    const DEPTH_LAYERS = 1; // Single layer wall
    ...
    const animate = () => {
      ...
      camera.position.x = 1.5 * Math.sin(elapsed * 0.15);
      camera.position.y = 1.0 * Math.sin(elapsed * 0.12 + 0.5);
      camera.lookAt(0, 0, 0);
