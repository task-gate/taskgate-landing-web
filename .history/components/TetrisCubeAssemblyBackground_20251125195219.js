...
const TetrisCubeAssemblyBackground = () => {
  ...
  useEffect(() => {
    ...
    for (let x = 0; x < GRID_SIZE; x++) {
      for (let y = 0; y < GRID_SIZE; y++) {
        for (let z = 0; z < GRID_SIZE; z++) {
          const isEye = x === eyeGridPos.x && y === eyeGridPos.y && z === eyeGridPos.z;
          allPieces.push({
            type: "cube",
            isEye,
    ...
    // Tail: square pyramid positioned behind the body (negative X direction)
    // The base should match the cube body face, tip pointing away
    const bodyFaceSize = GRID_SIZE * UNIT; // size of one face of the 3x3x3 body
    const tailBaseSize = bodyFaceSize * 0.85; // slightly smaller than body face
    const tailHeight = bodyFaceSize * 1.1; // length of the tail
    
    // Position tail so its base touches the back face of the body
    // Body back face is at x = bodyOffset - CUBE_SIZE/2
    // Pyramid tip points in -X, so we position the center of the pyramid
    const tailCenterX = bodyOffset - CUBE_SIZE / 2 - tailHeight / 2;
    
    allPieces.push({
      type: "tail",
      targetPos: new THREE.Vector3(tailCenterX, 0, 0),
    });
    ...
    const createFallingPiece = (pieceData, startTime) => {
      ...
      if (pieceData.type === "cube") {
        const mat = pieceData.isEye ? eyeMaterial.clone() : bodyMaterial.clone();
        if (!pieceData.isEye) {
        ...
        // Use a custom geometry: square base, 4 triangular faces meeting at apex
        const bodyFaceSize = GRID_SIZE * UNIT;
        const tBaseSize = bodyFaceSize * 0.85;
        const tHeight = bodyFaceSize * 1.1;
        
        // ConeGeometry with 4 radial segments creates a square pyramid
        // But we need to scale it properly - radius is half the diagonal
        const halfBase = tBaseSize / 2;
        const pyramidGeom = new THREE.ConeGeometry(halfBase * Math.SQRT2, tHeight, 4);
        
        mesh = new THREE.Mesh(pyramidGeom, tailMaterial.clone());
        
        // Rotate pyramid so:
        // - Tip points in -X direction (behind the fish body)
        // - Square base faces +X (towards the body)
        // - Base edges align with Y and Z axes
        mesh.rotation.z = -Math.PI / 2; // tip points -X
        mesh.rotation.y = Math.PI / 4;  // rotate 45° so square edges align with Y/Z
      }
      ...
      return {
        ...
        targetRot: pieceData.type === "tail"
          ? new THREE.Euler(0, Math.PI / 4, -Math.PI / 2) // tip points -X, base aligned
    ...
    const animate = () => {
      ...
      if (assemblyPhase === "falling" && fallingData) {
        const t = Math.min(1, (nowMs - fallingData.startTime) / fallingData.duration);
        ...
        if (fallingData.pieceType === "cube") {
          fallingData.mesh.rotation.x = fallingData.startRot.x * (1 - easeOutQuad(t));
          fallingData.mesh.rotation.y = fallingData.startRot.y * (1 - easeOutQuad(t));
          fallingData.mesh.rotation.z = fallingData.startRot.z * (1 - easeOutQuad(t));
        } else {
          // Tail rotation interpolation
          const rotT = easeOutQuad(t);
          fallingData.mesh.rotation.x = fallingData.startRot.x * (1 - rotT);
          fallingData.mesh.rotation.y =
            fallingData.startRot.y * (1 - rotT) + fallingData.targetRot.y * rotT;
          fallingData.mesh.rotation.z =
            fallingData.startRot.z * (1 - rotT) + fallingData.targetRot.z * rotT;
        }
        ...
        if (nowMs - phaseStartTime > WAIT_BETWEEN) {
          fallingData = createFallingPiece(shuffledPieces[currentPieceIndex], nowMs);
          currentFallingMesh = fallingData.mesh;
          assemblyPhase = "falling";
        }
        ...
        if (nowMs - phaseStartTime > COMPLETE_PAUSE) {
          ...
          assembledMeshes.forEach((item) => {
            const dir = item.targetPos.clone().normalize().multiplyScalar(8 + Math.random() * 4);
      ...
      // Gentle rotation of the whole fish
      fishGroup.rotation.y = elapsed * 0.12;
      fishGroup.rotation.x = 0.08 * Math.sin(elapsed * 0.18) - 0.1;

      // Subtle swim bob
      fishGroup.position.y = 0.15 * Math.sin(elapsed * 0.8);
