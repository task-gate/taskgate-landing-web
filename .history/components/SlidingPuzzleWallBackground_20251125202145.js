    ...
    const animate = () => {
      ...
      for (let i = activeSlides.length - 1; i >= 0; i--) {
        ...
        if (t >= 1) {
          ...
          // More noticeable depth pop during slide
          const depthPop = Math.sin(t * Math.PI) * 1.2;
          slide.cube.position.z = depthPop;
        }
      }

      // Subtle wall breathing - all cubes micro-move in Z
      for (const cube of allCubes) {
        if (!isSliding(cube)) {
          const noise = elapsed * 0.6 + cube.userData.gridRow * 0.25 + cube.userData.gridCol * 0.15;
          cube.position.z = 0.15 * Math.sin(noise);
        }
      }

      // Gentle camera orbit for depth feel
      const baseCamX = 25;
      const baseCamY = 18;
      const baseCamZ = 55;
      camera.position.x = baseCamX + 4 * Math.sin(elapsed * 0.08);
      camera.position.y = baseCamY + 3 * Math.sin(elapsed * 0.06 + 0.5);
      camera.position.z = baseCamZ + 3 * Math.sin(elapsed * 0.05);
      camera.lookAt(0, 0, 0);
