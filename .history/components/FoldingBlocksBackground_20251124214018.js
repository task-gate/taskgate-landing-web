    ...
    class FoldingBlock {
      ...
      update(time) {
        ...
        if (adjustedTime < FOLD_DURATION) {
          ...
          // Hold as cube
          this.foldProgress = 1;
        } else if (adjustedTime < FOLD_DURATION + HOLD_DURATION + UNFOLD_DURATION) {
          // Unfolding
          const unfoldTime = adjustedTime - FOLD_DURATION - HOLD_DURATION;
          this.foldProgress = 1 - this.easeInOutCubic(unfoldTime / UNFOLD_DURATION);
        } else {
          // Rest as flat
          this.foldProgress = 0;
        }
      ...
      drawFoldingTransition(ctx, progress) {
        ...
        ctx.moveTo(-halfSize, -halfSize * compression);
        ctx.lineTo(-halfSize, halfSize * compression);
        ctx.lineTo(-halfSize * (1 - progress * 0.5), halfSize * compression - liftHeight * 0.3);
        ctx.lineTo(-halfSize * (1 - progress * 0.5), -halfSize * compression - liftHeight * 0.3);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.moveTo(-halfSize * (1 - progress * 0.5), -halfSize * compression - liftHeight * 0.3);
        ctx.lineTo(halfSize * (1 - progress * 0.5), -halfSize * compression - liftHeight * 0.3);
        ctx.lineTo(halfSize * (1 - progress * 0.5), halfSize * compression - liftHeight * 0.3);
        ctx.lineTo(-halfSize * (1 - progress * 0.5), halfSize * compression - liftHeight * 0.3);
        ctx.closePath();
