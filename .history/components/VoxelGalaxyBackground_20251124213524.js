  ...
  useEffect(() => {
    ...
    class Voxel {
      ...
      draw(ctx, centerX, centerY, opacity = 1) {
        ...
        // Top face (lighter)
        ctx.fillStyle = `rgba(${Math.min(255, r + 40)}, ${Math.min(
          255,
          g + 40
        )}, ${Math.min(255, b + 40)}, ${opacity})`;
        ctx.beginPath();
        ...
        // Right face (darker)
        ctx.fillStyle = `rgba(${Math.max(0, r - 40)}, ${Math.max(
          0,
          g - 40
        )}, ${Math.max(0, b - 40)}, ${opacity})`;
        ctx.beginPath();
    ...
    for (let i = 0; i < 150; i++) {
      ...
      sunVoxels.push(
        new Voxel(x, y, z, sunVoxelSize, "#FF8C00", targetX, targetY, targetZ)
      );
    }
    ...
    planets.forEach((planet, planetIndex) => {
      ...
      for (let i = 0; i < numVoxels; i++) {
        ...
        const targetR =
          planet.distance * 0.8 + (Math.random() - 0.5) * planet.size * 2;
    ...
    const animate = () => {
      ...
      if (
        time >= TRANSFORM_START &&
        time < TRANSFORM_START + TRANSFORM_DURATION
      ) {
      ...
      allVoxels.forEach((group) => {
        if (group.type === "sun") {
          ...
          group.voxels.forEach((voxel) => {
            ...
            const worldX = transformProgress < 1 ? orbitX + voxel.x : voxel.x;
            const worldY = transformProgress < 1 ? orbitY + voxel.y : voxel.y;
