...
const SlidingCubeGridBackground = () => {
  ...
  useEffect(() => {
    ...
    // Parent group that represents the fish-shaped logo in 3D
    const fishGroup = new THREE.Group();
    scene.add(fishGroup);

    // Body: sliding cube grid lives inside this child group so we can
    // still reuse the existing slide logic unchanged.
    const cubeGroup = new THREE.Group();
    fishGroup.add(cubeGroup);
    ...
    for (let xi = 0; xi < GRID_SIZE; xi++) {
      for (let yi = 0; yi < GRID_SIZE; yi++) {
        for (let zi = 0; zi < GRID_SIZE; zi++) {
          ...
          // Subtle accent on some front/top voxels to suggest a "head"
          const isHeadRegion = lz === 1 && ly >= 0;
          if (isHeadRegion) {
            cube.material.color.setHex(0xa8b3ff);
    ...
    // Tail: a subtle triangular cone attached to the back-right/top area
    ...
    const tailGeometry = new THREE.ConeGeometry(0.7, 1.6, 3);
    const tailMesh = new THREE.Mesh(tailGeometry, tailMaterial);
    // Position slightly behind and above the cube cluster, pointing backward
    tailMesh.position.set(1.2 * SPACING, 0.6 * SPACING, -2.2 * SPACING);
    tailMesh.rotation.x = Math.PI / 2;
    fishGroup.add(tailMesh);

    // Eye: small glowing sphere on the front-left of the body
    ...
    eyeMesh.position.set(-0.7 * SPACING, 0.3 * SPACING, 1.8 * SPACING);
    ...
    const animate = () => {
      ...
      // Very slow fish rotation for calm parallax
      fishGroup.rotation.y = 0.18 * Math.sin(elapsed * 0.08);
      fishGroup.rotation.x = 0.12 * Math.sin(elapsed * 0.06) - 0.2;
